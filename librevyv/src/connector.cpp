...
uint32_t Connector::window_create(unsigned char* data, size_t size, double x, double y, double width, double height, WindowRasterType raster_type)
{
    ...
    _listener->send_listener_payload(WindowCreatePayload(w.id, x, y, width, height, raster_type, static_cast<uint64_t>(size), w.shared_memory_id).get_payload());
...
void Connector::window_update_pixels(uint32_t window_id, unsigned char* data, size_t size, double x, double y, double width, double height)
{
    ...
    try {
        ...
        _listener->send_listener_payload(WindowUpdatePixelsPayload(window_id, x, y, width, height, static_cast<uint64_t>(size), true, compressor.getSize(), w.shared_memory_id).get_payload());
    } catch (FailedToCompressDataError& e) {
        /* We failed to compress data, send it uncompressed. */
        std::memcpy(w.shared_memory, data, size);
        _listener->send_listener_payload(WindowUpdatePixelsPayload(window_id, x, y, width, height, static_cast<uint64_t>(size), false, static_cast<uint64_t>(size), w.shared_memory_id).get_payload());
    }
}

void Connector::window_resize(uint32_t window_id, unsigned char* data, size_t size, double width, double height)
{
    auto w = _windows[window_id];
    _listener->send_listener_payload(WindowResizePayload(w.id, width, height, static_cast<uint64_t>(size), w.shared_memory_id).get_payload());
